...
export const Name = ({ db, userId }: {
    db: Database;
    userId?: string;
}) => <>{db.users.find((user) => user.id === userId)?.name ?? '—'}</>;
export const UnitName = ({ db, unitId }: {
    db: Database;
    unitId?: string;
...
function AcknowledgementBadge({ db, protocol }: {
    db: Database;
    protocol: Protocol;
}) {
    const assignment = db.assignments.find((item) => item.id === protocol.currentAssignmentId);
    if (!protocol.currentAssigneeId)
        return <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Aguardando responsável</span>;
    if (isActive(protocol) && !assignment?.receivedAt)
        return <span role="status" className="inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-900 dark:bg-amber-950 dark:text-amber-200">Ciência pendente</span>;
    return <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">Ciência registrada</span>;
}
export function ProtocolTable({ db, protocols }: {
    db: Database;
    protocols: Protocol[];
}) {
    return <>
