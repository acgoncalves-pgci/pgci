...
import { Archive, ArrowLeft, ArrowRight, Building2, CalendarClock, Check, CheckCheck, CheckCircle2, ChevronDown, ChevronRight, ChevronUp, ClipboardList, Clock3, Eye, FileArchive, FilePlus2, FileText, GitBranch, Grid3X3, ListFilter, Paperclip, Pencil, Plus, Printer, RefreshCcw, Search, Trash2, UserRound, UserRoundCog, UsersRound } from 'lucide-react';
...
export function Protocols() {
    ...
    const filters: ProtocolFilters = {
        tab: (params.get('tab') as ProtocolFilters['tab']) || 'mine', search: params.get('search') || '', shortcut: (params.get('shortcut') as ProtocolFilters['shortcut']) || '', statuses,
    ...
    const { data, isLoading, error } = useQuery({ queryKey: ['processes', ctx.userId, ctx.activeUnitId, location.search], queryFn: () => api.listProtocols(ctx, filters), placeholderData: (previous) => previous });
    const { data: db } = useDb();
    ...
    const advancedCount = [filters.typeId, filters.interestedId, filters.creditorId, filters.unitId, filters.assigneeId, filters.createdFrom, filters.createdTo, filters.number, filters.description, filters.attachments, filters.statuses?.length ? 'status' : ''].filter(Boolean).length;
    return <>
...
type AdvancedFilterDraft = {
    typeId: string; interestedId: string; creditorId: string; unitId: string; assigneeId: string; statuses: ProtocolStatus[];
...
function AdvancedProcessFilterDialog({ db, filters, onClose, onApply }: { db: Database; filters: ProtocolFilters; onClose: () => void; onApply: (values: Record<string, string>) => void }) {
    const initial: AdvancedFilterDraft = { typeId: filters.typeId ?? '', interestedId: filters.interestedId ?? '', creditorId: filters.creditorId ?? '', unitId: filters.unitId ?? '', assigneeId: filters.assigneeId ?? '', statuses: filters.statuses ?? [], createdFrom: filters.createdFrom ?? '', createdTo: filters.createdTo ?? '', number: filters.number ?? '', description: filters.description ?? '', attachments: filters.attachments ?? '' };
    ...
    const toggleStatus = (status: ProtocolStatus) => update('statuses', draft.statuses.includes(status) ? draft.statuses.filter((item) => item !== status) : [...draft.statuses, status]);
    const clear = () => setDraft({ typeId: '', interestedId: '', creditorId: '', unitId: '', assigneeId: '', statuses: [], createdFrom: '', createdTo: '', number: '', description: '', attachments: '' });
    const apply = () => onApply({ typeId: draft.typeId, interestedId: draft.interestedId, creditorId: draft.creditorId, unitId: draft.unitId, assigneeId: draft.assigneeId, status: draft.statuses.join(','), createdFrom: draft.createdFrom, createdTo: draft.createdTo, number: draft.number, description: draft.description, attachments: draft.attachments });
    return <Dialog title="Pesquisar processos" onClose={onClose} wide><form onSubmit={(event) => { event.preventDefault(); apply(); }} className="process-advanced-form">
      ...
      <fieldset><legend className="mb-2 text-xs font-bold">Situação do processo</legend><div className="flex flex-wrap gap-2">{Object.entries(statusLabel).map(([status, label]) => <button key={status} type="button" aria-pressed={draft.statuses.includes(status as ProtocolStatus)} className="process-status-filter" onClick={() => toggleStatus(status as ProtocolStatus)}>{draft.statuses.includes(status as ProtocolStatus) && <Check size={13}/>} {label}</button>)}</div></fieldset>
...
export function NewProtocol() {
    ...
    if (ctx.activeUnitId !== ctx.user.unitId)
        return <><PageTitle title="Abrir processo"/><ErrorBox error={new Error('Para abrir processo, selecione sua unidade de vínculo.')}/></>;
    ...
    const responsibleUsers = db.users.filter((user) =>
        user.active && db.memberships.some((membership) =>
            membership.userId === user.id && membership.unitId === ctx.activeUnitId && membership.active,
        ),
    );
...
function ProcessSummary({ protocol, db, phaseName, latestMovement }: { protocol: Protocol; db: Database; phaseName?: string; latestMovement?: ProtocolEvent }) {
    ...
    return <section className="panel mt-3 overflow-hidden">
      ...
      <SummarySection title="Situação atual" divided>
        <SummaryField label="Situação" value={statusLabel[protocol.status]}/>
...
function TimelineRow({ event: e, db, protocol, assignment, attachments, documents, isLast, isLatest, latestEventId, canAcknowledge, canAssign, canEdit, onConfirmAcknowledge, onAssign, onAttachFile, onReceipt, onDossier }: {
    event: ProtocolEvent;
    db: Database;
    protocol: Protocol;
    assignment: Database['assignments'][number];
    ...
    const heading = e.kind === 'ABERTURA' ? 'Abertura do processo' : eventLabel[e.kind];
    ...
    return <article className="relative flex gap-3 pb-2 last:pb-0 sm:gap-4">
      ...
      <div data-timeline-dot className={`relative z-10 mt-3.5 hidden h-3 w-3 shrink-0 rounded-full ring-4 sm:block ${dotClass}`}/>
      ...
      <section className="min-w-0 flex-1 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-slate-700 dark:bg-slate-900">
        <div className="flex min-h-10 w-full items-center gap-2 px-3 py-1.5">
          <button type="button" className="flex min-w-0 flex-1 items-center gap-2 text-left" aria-expanded={!collapsed} aria-controls={contentId} aria-label={`${collapsed ? 'Expandir' : 'Recolher'} conteúdo de ${heading}`} onClick={() => setCollapsed((value) => !value)}>
            <span className="flex min-w-0 flex-wrap items-center gap-2"><strong className="text-sm text-slate-800 dark:text-slate-100">{heading}</strong><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${badgeClass}`}>{status}</span></span>
          </button>
          {isLatest && isActive(protocol) && protocol.currentAssigneeId && <span role="group" aria-label="Ciência e responsável" className="timeline-inline-actions">
        ...
        {!collapsed && <div id={contentId} className="border-t border-slate-100 dark:border-slate-800">
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
            <div className="flex min-w-0 flex-wrap items-center gap-3 text-sm">
              ...
              {canShowRoute && destination && <><ArrowRight className="shrink-0 text-slate-400" size={16}/><span className="inline-flex min-w-0 items-center gap-2 font-medium text-slate-700 dark:text-slate-200"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-emerald-50 text-emerald-700 dark:bg-slate-800 dark:text-emerald-300"><UserRound size={15}/></span><span className="truncate">{destination}</span></span></>}
            </div>
