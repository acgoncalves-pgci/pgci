...
export const Switch = forwardRef<HTMLButtonElement, SwitchProps>(({ checked, defaultChecked = false, disabled = false, className = '', onChange, onCheckedChange, ...props }, ref) => {
  const [uncontrolledChecked, setUncontrolledChecked] = useState(defaultChecked); const isControlled = checked !== undefined; const currentChecked = isControlled ? checked : uncontrolledChecked
  const toggle = () => { if (disabled) return; const nextChecked = !currentChecked; if (!isControlled) setUncontrolledChecked(nextChecked); onCheckedChange?.(nextChecked); onChange?.({ target: { checked: nextChecked }, currentTarget: { checked: nextChecked } } as ChangeEvent<HTMLInputElement>) }
