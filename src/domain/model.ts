...
export interface Database { schemaVersion: 5; initializedAt: string; organization: { id: string; name: string; abbreviation: string }; counters: Record<string, number>; units: Unit[]; users: AppUser[]; memberships: UserUnitMembership[]; auditEvents: AuditEvent[]; people: Person[]; processCategories: ProcessCategory[]; protocolTypes: ProtocolType[]; phases: ProtocolPhase[]; flows: ProtocolFlow[]; flowPhases: FlowPhase[]; situations: SituationType[]; documentTypes: DocumentType[]; protocols: Protocol[]; assignments: Assignment[]; events: ProtocolEvent[]; documents: AppDocument[]; attachments: Attachment[] }
