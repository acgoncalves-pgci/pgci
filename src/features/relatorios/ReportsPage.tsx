    ...
    <section id={`report-panel-${tab}`} role="tabpanel" aria-labelledby={`report-tab-${tab}`} className="reports-panel">
      <form onSubmit={(event) => { event.preventDefault(); void generate(); }}>
        {tab === 'processes' && <div className="grid gap-x-4 gap-y-5 md:grid-cols-3">
          ...
          <ReportField label="Relatório"><Select value={listTemplate} onChange={(e) => setListTemplate(e.target.value)}><option value="list">Listagem de Processos</option><option value="summary">Resumo de Processos</option></Select></ReportField>
        </div>}
        {tab === 'individual' && <div className="grid gap-4 md:grid-cols-2">
          <ReportField label="Número do processo"><Input placeholder="Informe o número completo" value={number} onChange={(e) => setNumber(e.target.value)} /><small className="mt-2 block text-xs text-muted-foreground">{number.trim() ? individual ? individual.subject : 'Processo não encontrado no seu escopo.' : 'Digite o número do processo para gerar a capa do processo.'}</small></ReportField>
