...
export function seedDatabase(): Database {
    const units = [
        { id: 'u-prot', name: 'Protocolo Geral', abbreviation: 'PG', position: 0, active: true },
    ...
    const flows: Database['flows'] = [{ id: 'flow-standard-v1', name: 'Fluxo padrão de protocolos', version: 1, active: true, startsAt: now() }];
    const flowPhases: Database['flowPhases'] = phases.map((phase, index) => ({ id: `flow-phase-standard-${index + 1}`, flowId: 'flow-standard-v1', phaseId: phase.id, position: index, required: true }));
    const protocolTypes = [
    ...
    ].map(([id, name, description, color, defaultDeadlineDays, fieldsConfig]) => ({ id: id as string, name: name as string, description: description as string, color: color as string, defaultDeadlineDays: defaultDeadlineDays as number, fieldsConfig: fieldsConfig as FieldsConfig, flowId: 'flow-standard-v1', active: true }));
    ...
    const flowSnapshot = {
        ...
        phases: flowPhases.map((flowPhase) => {
            ...
            return { phaseId: phase.id, name: phase.name, code: phase.code, position: flowPhase.position, required: flowPhase.required, eligibleUnitIds: phase.eligibleUnitIds, checklistItems: phase.checklistItems, checklistQuestions: phase.checklistQuestions ?? [], requiredAttachmentTypes: phase.requiredAttachmentTypes };
        }),
    };
    ...
    return { schemaVersion: 3, initializedAt: now(), organization: { id: 'org-1', name: 'Prefeitura de Vila Exemplo', abbreviation: 'PVE' }, counters: { 'protocol-2026': 20, 'document-2026': 6 }, units, users, memberships, auditEvents, people, protocolTypes, phases, flows, flowPhases, documentTypes, protocols, assignments, events, documents, attachments: [{ id: 'att-seed', protocolId: 'pr-1', filename: 'comprovante-demo.txt', mimeType: 'text/plain', sizeBytes: 52, blobKey: 'seed-comprovante', uploadedById: 'usr-clara', createdAt: isoDaysFromNow(-1) }] };
}
