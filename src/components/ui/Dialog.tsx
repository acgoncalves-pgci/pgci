...
export function Dialog({ title, children, onClose, wide = false }: { title: string; children: ReactNode; onClose: () => void; wide?: boolean }) {
  const contentRef = useRef<HTMLElement>(null); const openerRef = useRef<HTMLElement | null>(null); const closeTimer = useRef<number>(); const titleId = useId(); const [closing, setClosing] = useState(false)
  useEffect(() => {
    ...
    return () => { window.cancelAnimationFrame(frame); if (closeTimer.current) window.clearTimeout(closeTimer.current); openerRef.current?.focus() }
  }, [])
  const requestClose = () => { if (closing) return; setClosing(true); closeTimer.current = window.setTimeout(onClose, 160) }
...
export function PrintPreviewDialog({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
