...
import {
  eventLabel,
  isActive,
  isLegacyAssumptionEvent,
  isMovementEvent,
  statusLabel,
} from "../../domain/model";
import {
  currentProtocolSituation,
  legacySituationTypeId,
} from "../../domain/situations";
import {
  canAct as canActProtocol,
  canOpenProtocolType,
...
export function Protocols() {
  ...
  const statuses = (params.get("status") ?? "")
    .split(",")
    .filter((status): status is ProtocolStatus => status in statusLabel);
  const situationIds = (params.get("situation") ?? "")
    .split(",")
    .filter(Boolean);
  const filters: ProtocolFilters = {
    tab: (params.get("tab") as ProtocolFilters["tab"]) || "mine",
    search: params.get("search") || "",
    shortcut: (params.get("shortcut") as ProtocolFilters["shortcut"]) || "",
    statuses,
    situationIds,
    ...
    attachments:
      (params.get("attachments") as ProtocolFilters["attachments"]) || "",
    sort: (params.get("sort") as ProtocolFilters["sort"]) || "updated-desc",
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => api.listProtocols(ctx, filters),
  ...
  const quickFilters = [
    {
      id: "mine",
      label: "Para mim",
      detail: "Processos endereçados a você.",
      icon: UserRound,
    },
    {
      id: "created",
      label: "Gerados por mim",
      detail: "Processos abertos por você.",
      icon: FileText,
    },
    {
      id: "unit",
      label: "Minha Unidade",
      detail: data?.unassignedInUnit
        ? `${data.unassignedInUnit} processo(s) sem destinatário`
        : "Processos da sua unidade.",
      icon: Building2,
    },
    {
      id: "participated",
      label: "Já participei",
      detail: "Processos em que você participou.",
      icon: UsersRound,
    },
    {
      id: "all",
      label: "Todos",
      detail: "Processos disponíveis no seu escopo.",
      icon: Grid3X3,
    },
  ] as const;
  const shortcuts = [
  ...
  const advancedCount = [
    ...
    filters.description,
    filters.attachments,
    filters.situationIds?.length || filters.statuses?.length ? "situation" : "",
    ...
    <>
      ...
      <section className="no-print process-search-panel">
        <div className="process-search-row">
          <label className="process-search-input">
            <Search aria-hidden="true" size={18} />
            <Input
              aria-label="Buscar processos"
              className="!mt-0 !border-0 !bg-transparent !px-0 !shadow-none"
              placeholder="Buscar por número, assunto ou interessado..."
              value={filters.search}
              onChange={(event) => set({ search: event.target.value }, true)}
            />
          </label>
          <button
            type="button"
            className="btn-secondary process-advanced-button"
            onClick={() => setAdvancedOpen(true)}
          >
            <ListFilter size={16} />
            Filtro avançado
            {advancedCount ? (
              <span className="process-filter-count">{advancedCount}</span>
            ) : null}
          </button>
        </div>
        <div className="process-quick-filters" aria-label="Filtros rápidos">
          {quickFilters.map(({ id, label, detail, icon: Icon }) => (
            <button
              key={id}
              type="button"
              aria-pressed={filters.tab === id}
              className="process-quick-filter"
              onClick={() => set({ tab: id, shortcut: "" })}
            >
        ...
        <div className="process-shortcuts">
          ...
          {shortcuts.map(([id, label]) => (
            <button
              key={id}
              type="button"
              aria-pressed={filters.shortcut === id}
              onClick={() => set({ shortcut: id })}
              className="process-shortcut"
            >
              {label}
            </button>
          ))}
...
function AdvancedProcessFilterDialog({
  db,
  filters,
  onClose,
  onApply,
  ...
  const legacySituationIds = (filters.statuses ?? [])
    .map(legacySituationTypeId)
    .filter((id): id is string => Boolean(id));
  const initial: AdvancedFilterDraft = {
  ...
  const update = <K extends keyof AdvancedFilterDraft>(
    key: K,
    value: AdvancedFilterDraft[K],
  ) => setDraft((current) => ({ ...current, [key]: value }));
  const toggleSituation = (situationId: string) =>
    update(
      "situationIds",
      draft.situationIds.includes(situationId)
        ? draft.situationIds.filter((item) => item !== situationId)
        : [...draft.situationIds, situationId],
    );
  const clear = () =>
    setDraft({
      typeId: "",
  ...
  return (
    <Dialog title="Pesquisar processos" onClose={onClose} wide>
      <form
        onSubmit={(event) => {
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Unidade organizacional">
            ...
            >
              ...
              {sortUnitsByPath(db.units.filter((unit) => unit.active)).map(
                (unit) => (
                  <option key={unit.id} value={unit.id}>
                    {unitPath(db.units, unit.id)}
                  </option>
                ),
              )}
            </Select>
        ...
        <fieldset>
          ...
          <div className="flex flex-wrap gap-2">
            {db.situations
              .filter(
                (situation) =>
                  situation.active || draft.situationIds.includes(situation.id),
              )
              .map((situation) => (
                <button
                  key={situation.id}
                  type="button"
                  aria-pressed={draft.situationIds.includes(situation.id)}
                  className="process-status-filter"
                  onClick={() => toggleSituation(situation.id)}
                >
                  {draft.situationIds.includes(situation.id) && (
                    <Check size={13} />
                  )}{" "}
                  {situation.name}
                </button>
        ...
        <fieldset>
          <legend className="mb-2 text-xs font-bold">Anexos</legend>
          <div className="flex flex-wrap gap-2">
            {[
              ["", "Todos"],
              ["with", "Com anexos"],
              ...
              <button
                key={value}
                type="button"
                aria-pressed={draft.attachments === value}
                className="process-attachment-filter"
                onClick={() =>
                  update(
                    "attachments",
                    value as AdvancedFilterDraft["attachments"],
                  )
                }
              >
                {draft.attachments === value && <Check size={13} />} {label}
              </button>
            ))}
          </div>
        </fieldset>
...
const protocolSchema = z.object({
  ...
  description: z
    ...
    .max(4000, "A descrição deve ter até 4.000 caracteres."),
  observations: z
    .string()
    .trim()
  ...
  contractNumber: z
    .string()
    .trim()
    .max(120, "O número de contrato deve ter até 120 caracteres.")
    .optional(),
  biddingNumber: z
    .string()
    .trim()
    .max(120, "O número de licitação deve ter até 120 caracteres.")
    .optional(),
  legalProcessNumber: z
    .string()
    .trim()
    .max(120, "O número do processo jurídico deve ter até 120 caracteres.")
    .optional(),
  referenceNumber: z
    .string()
    .trim()
    .max(120, "O número deve ter até 120 caracteres.")
    .optional(),
...
export function NewProtocol() {
  ...
  const create = useMutation({
    mutationFn: (data: ProtocolFormData & { subject: string }) =>
  ...
  const selectType = (nextTypeId: string) => {
    ...
    if (!nextFields.contractNumber?.enabled)
      form.setValue("contractNumber", "");
    if (!nextFields.biddingNumber?.enabled) form.setValue("biddingNumber", "");
    if (!nextFields.legalProcessNumber?.enabled)
    ...
    form.setValue(
      "dueAt",
      nextType.defaultDeadlineDays
        ? suggestedDeadline(nextType.defaultDeadlineDays)
        : "",
    );
    form.clearErrors();
    setFileError("");
  ...
  if (isLoading || !db || !ctx.user) return <Loading variant="detail" />;
  const activeMembership = findActiveMembershipForUnit(
    db,
    ctx.userId,
    ctx.activeUnitId,
  );
    ...
    return (
      <>
        ...
        <ErrorBox
          error={
            new Error(
              "Para abrir processo, selecione uma unidade com vínculo ativo.",
            )
          }
        />
      </>
    ...
    return (
      <>
        ...
        <ErrorBox
          error={
            new Error(
              "Seu vínculo com a unidade selecionada permite somente leitura.",
            )
          }
        />
      </>
  ...
  const hasContractNumber = Boolean(fields?.contractNumber?.enabled);
  const hasBiddingNumber = Boolean(fields?.biddingNumber?.enabled);
  const hasLegalProcessNumber = Boolean(fields?.legalProcessNumber?.enabled);
  const hasReferenceNumber = Boolean(fields?.referenceNumber?.enabled);
  ...
  const configuredFlow = type?.flowId
    ? db.flows.find((flow) => flow.id === type.flowId && flow.active)
    : undefined;
  const configuredStages = configuredFlow
    ? db.flowPhases
        .filter((stage) => stage.flowId === configuredFlow.id)
        .sort((left, right) => left.position - right.position)
    : [];
  const flowReady = Boolean(
    configuredFlow &&
    configuredStages.length &&
    configuredStages.every((stage) =>
      db.phases.some((phase) => phase.id === stage.phaseId && phase.active),
    ),
  );
  ...
  const submit = (data: ProtocolFormData) => {
    ...
    requireValue(
      "interestedPersonId",
      Boolean(fields?.interested.required),
      "Selecione o interessado.",
    );
    requireValue(
      "creditorPersonId",
    ...
    requireValue(
      "contractNumber",
      Boolean(fields?.contractNumber?.required),
      "Informe o número de contrato.",
    );
    requireValue(
      "biddingNumber",
      Boolean(fields?.biddingNumber?.required),
      "Informe o número de licitação.",
    );
    requireValue(
      "legalProcessNumber",
      Boolean(fields?.legalProcessNumber?.required),
      "Informe o número do processo jurídico.",
    );
    requireValue(
      "referenceNumber",
      Boolean(fields?.referenceNumber?.required),
      "Informe o número.",
    );
    ...
    create.mutate({
      ...data,
      subject: hasSubject ? data.subject.trim() : type.name,
    });
  };
    ...
    <>
      ...
      >
        <section className="panel p-5">
          ...
          {!type && (
            <p className="mt-4 rounded-lg border border-dashed border-border px-3 py-4 text-sm text-muted-foreground">
              Selecione o tipo de processo para ver os campos disponíveis.
            </p>
          )}
        </section>
              ...
              <section className="panel p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  {flowMode === "REQUIRED" ? (
                    ...
                    <label className="flex items-center gap-3 rounded-lg border border-border px-3 py-2 text-sm font-semibold">
                      <Switch
                        aria-label="Aplicar fluxo sugerido"
                        checked={useSuggestedFlow && flowReady}
                        disabled={!flowReady}
                        onCheckedChange={setUseSuggestedFlow}
                      />
                ...
                {flowReady ? (
                  <ol
                    ...
                    aria-label="Fases configuradas"
                  >
                    {configuredStages.map((stage, index) => {
                      const configuredPhase = db.phases.find(
                        (phase) => phase.id === stage.phaseId,
                      );
                      return (
                        <li
                          key={stage.id}
                        ...
                        >
                          ...
                          <span className="font-semibold">
                            {configuredPhase?.name}
                          </span>
                          {stage.destinationUnitId && (
                            <span className="text-xs text-muted-foreground">
                              →{" "}
                              {
                                db.units.find(
                                  (unit) => unit.id === stage.destinationUnitId,
                                )?.name
                              }
                            </span>
                          )}
            ...
            <section className="panel p-5">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                {hasBiddingNumber && (
                  <Field
                    label={
                      "Número de licitação" +
                      (fields?.biddingNumber?.required ? " *" : "")
                    }
                    error={form.formState.errors.biddingNumber?.message}
                  >
                    <Input
                      className="field"
                      maxLength={120}
                      placeholder="Informe o número da licitação"
                      {...form.register("biddingNumber")}
                    />
                  </Field>
                )}
                {hasLegalProcessNumber && (
                  <Field
                    label={
                      "Número de processo jurídico" +
                      (fields?.legalProcessNumber?.required ? " *" : "")
                    }
                    error={form.formState.errors.legalProcessNumber?.message}
                  >
                    <Input
                      className="field"
                      maxLength={120}
                      placeholder="Informe o número do processo jurídico"
                      {...form.register("legalProcessNumber")}
                    />
                  </Field>
                )}
                {hasReferenceNumber && (
                  <Field
                    label={
                      "Número" + (fields?.referenceNumber?.required ? " *" : "")
                    }
              ...
              <div className="mt-4">
                ...
                >
                  <textarea
                    ...
                    maxLength={4000}
                    placeholder="Informe uma descrição para este processo"
                    {...form.register("description")}
                  />
                </Field>
                <p className="mt-1 text-xs text-muted-foreground">
                  Caracteres restantes:{" "}
                  {4000 - form.watch("description").length}
                </p>
              </div>
              <div className="mt-4">
                <Field
              ...
              {hasFiles && (
                <div className="mt-4">
                  <Field
                    label={
                      "Arquivos" + (fields?.arquivos?.required ? " *" : "")
                    }
                    error={fileError}
                  >
              ...
              {hasPortal && (
                <p className="mt-4 rounded-lg border border-border bg-muted/40 px-3 py-2 text-sm text-muted-foreground">
                  Este tipo permite acompanhamento pelo portal do cidadão.
                </p>
              )}
            </section>
          </>
        ...
        <div className="flex justify-end gap-2 border-t border-border pt-4">
          <Link className="btn-secondary" to="/processos">
            Cancelar
          </Link>
          <button
            className="btn-primary"
            disabled={!type || !requiredFlowReady || create.isPending}
          >
...
function PersonQuickDialog({
  ...
  return (
    <Dialog title="Cadastrar pessoa" onClose={onClose}>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          create.mutate();
      ...
      >
        <Field label="Tipo">
          <Select
            className="field"
            value={kind}
            onChange={(e) => setKind(e.target.value as "PF" | "PJ")}
          >
            <option value="PF">Pessoa física</option>
            <option value="PJ">Pessoa jurídica</option>
          </Select>
        </Field>
        <Field label={kind === "PF" ? "Nome completo *" : "Razão social *"}>
          <Input
            className="field"
            autoFocus
            value={name}
...
export function ProtocolDetail() {
  ...
  const [tab, setTab] = useState<
    "progress" | "data" | "documents" | "attachments" | "audit"
  >("progress");
  const [action, setAction] = useState<
    "forward" | "assign" | "complete" | "archive" | "reopen" | null
  >(null);
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["protocol", id, ctx.userId, ctx.activeUnitId],
    queryFn: () => api.getProtocol(ctx, id),
  });
  const refresh = () => invalidateAll(queryClient);
  const doAcknowledge = useMutation({
    mutationFn: () => api.acknowledge(ctx, id, data!.protocol.version),
    onSuccess: refresh,
  });
  ...
  const movementEvents = data.events.filter(
    (event) => isMovementEvent(event) && !isLegacyAssumptionEvent(data.events, event),
  );
  const flowPhaseByEventId = mapFlowPhasesToEvents(p, movementEvents);
  const checklistEventIds = mapChecklistEvents(
    movementEvents,
    flowPhaseByEventId,
  );
  ...
  if (p.status === "CONCLUIDO")
    primary = canAct && (
      <button className="btn-primary" onClick={() => setAction("archive")}>
        <Archive size={16} />
        Arquivar
      </button>
    );
  ...
  const printProcess = async (
    printAction: (typeof printActions)[number][0],
  ) => {
    setActionsOpen(false);
  ...
  const attachTimelineFiles = async (files: File[]) => {
    ...
    try {
      await api.addAttachments(
        ...
        p.version,
        files,
        timelineMovementId ?? latestMovement?.id,
      );
      refresh();
    } catch (error) {
      setCoverError(error);
      ...
      setTimelineMovementId(undefined);
    }
  };
  ...
  return (
    <>
      {!isHistoricalReadOnly && p.typeConfigSnapshot.arquivos?.enabled && (
        <input
          ...
          onChange={(event) => {
            ...
            void attachTimelineFiles(files);
      ...
      <div className="print-shell">
              ...
              {!isHistoricalReadOnly && (
                <div className="relative" ref={actionsRef}>
                  ...
                  {actionsOpen && (
                    <div
                      role="menu"
                      className="absolute right-0 z-30 mt-2 w-56 overflow-hidden rounded-xl border border-border bg-popover py-2 shadow-xl"
                    >
                      ...
                      <button
                        role="menuitem"
                        className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-muted disabled:cursor-not-allowed disabled:opacity-45"
                        disabled={!canEdit}
                        title={
                          !isActive(p)
                      ...
                      <button
                        role="menuitem"
                        className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-destructive hover:bg-muted disabled:cursor-not-allowed disabled:opacity-45"
                        disabled={!canDelete}
                        title={
                          !canDelete
                      ...
                      {printActions.map(([printAction, label]) => (
                        <button
                          key={printAction}
                          role="menuitem"
                          className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-muted"
                          disabled={printing}
                          onClick={() => void printProcess(printAction)}
                        >
                          <Printer size={16} />
                          {label}
                        </button>
        ...
        />
        {coverError ? <ErrorBox error={coverError} /> : null}
        {printing && (
          <p role="status" className="mb-4 text-sm text-muted-foreground">
            Gerando PDF do processo…
          </p>
        )}
        {isHistoricalReadOnly && (
          <UnitContextNotice
            unitName={processUnit?.name}
            canSwitch={canSwitchToProcessUnit}
            onSwitch={() => void switchToProcessUnit()}
          />
        )}
        <h2 className="sr-only">{p.subject}</h2>
        <p className={`${isHistoricalReadOnly ? '' : '-mt-4 '}mb-4 text-sm text-muted-foreground`}>{p.subject}</p>
        ...
        />
        <div className="no-print flex gap-1 overflow-x-auto rounded-lg border border-border bg-muted/60 p-1">
          {tabs.map(([key, label, Icon]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`inline-flex shrink-0 items-center gap-2 rounded-md px-3 py-2 text-sm font-semibold transition ${tab === key ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:bg-card/70 hover:text-foreground"}`}
            >
              {Icon}
              {label}
              {key === "progress" && (
              ...
              {key === "audit" && (
                <span className="rounded-full bg-muted px-1.5 py-0.5 text-[10px]">
                  {data.events.length + auditEvents.length}
                </span>
              )}
            </button>
          ))}
        ...
        {tab === "progress" && (
          <section className="timeline-panel panel mt-3 overflow-hidden">
            ...
            <div className="px-3 py-3 sm:px-5">
              {movementEvents.map((event, index) => (
                <TimelineRow
                  key={event.id}
                  event={event}
                  flowPhase={flowPhaseByEventId.get(event.id)}
                  db={db}
                  protocol={p}
                  assignment={assignment}
                  attachments={data.attachments.filter(
                    (item) => item.movementEventId === event.id,
                  )}
                  documents={data.documents.filter(
                    (item) => item.movementEventId === event.id,
                  )}
                  isLast={index === movementEvents.length - 1}
                  isLatest={index === 0}
                  latestEventId={latestMovement?.id}
                  canAcknowledge={
                    unacknowledged && p.currentAssigneeId === ctx.userId
                  }
                  canAssign={canManage}
                  ...
                  onConfirmAcknowledge={() => setConfirmAcknowledge(true)}
                  onAssign={() => setAction("assign")}
                  onAttachFile={() => {
                    setTimelineMovementId(event.id);
                    timelineFileInput.current?.click();
                  }}
                  onReceipt={(movement) =>
      ...
      {action === "archive" && (
        <ConfirmDialog
          ...
          body="O processo concluído deixará as filas de trabalho. Você poderá reabri-lo como administrador."
          confirm="Arquivar"
          onClose={() => setAction(null)}
          onConfirm={async () => {
            await api.archive(ctx, p.id, p.version);
            setAction(null);
            refresh();
          }}
        />
      ...
      {deleting && (
        <ConfirmDialog
          ...
          body="Esta ação exclui o processo e seus documentos, anexos e movimentações. Ela não pode ser desfeita."
          confirm="Excluir processo"
          danger
          onClose={() => setDeleting(false)}
          onConfirm={async () => {
            await api.deleteProtocol(ctx, p.id, p.version);
            navigate("/processos");
          }}
        />
      )}{" "}
      {confirmAcknowledge && (
        <ConfirmDialog
          ...
          body="Ao confirmar, a ciência desta tramitação será registrada em seu nome e o botão ficará inativo."
          confirm="Confirmar ciência"
          onClose={() => setConfirmAcknowledge(false)}
          onConfirm={async () => {
            await doAcknowledge.mutateAsync();
...
function UnitContextNotice({
  ...
  return (
    <div
      role="status"
      className={`process-unit-context ${canSwitch ? "process-unit-context--action" : ""}`}
    >
      {canSwitch ? (
      ...
      <p className="min-w-0 flex-1 truncate">
        <strong>
          {canSwitch ? "Troque a unidade para continuar" : "Somente leitura"}
        </strong>
        <span className="text-muted-foreground">
          {" "}
          · Processo em {unitName ?? "outra unidade"}.
          {!canSwitch && " Como você já participou dele, a consulta permanece disponível."}
        </span>
      </p>
      {canSwitch && (
        <button
          type="button"
          className="btn-primary !min-h-8 shrink-0 !px-3 !py-1.5 text-xs"
          onClick={onSwitch}
        >
          Alterar para {unitName}
          <ArrowRight size={14} />
        </button>
      )}
    </div>
...
function ProcessDetailOverview({
  protocol,
  db,
  phases,
  currentPhaseName,
  ...
  protocol: Protocol;
  db: Database;
  phases: NonNullable<Protocol["flowSnapshot"]>["phases"];
  ...
  const visiblePhases = phases.length
    ...
    : currentPhaseName
      ? [
          {
            phaseId: protocol.currentPhaseId ?? "manual",
            name: currentPhaseName,
            position: 0,
          },
        ]
      : [];
  const visiblePhaseIndex = phases.length
    ? phaseIndex
  ...
  return (
    <section
      className="process-detail-overview"
      aria-label="Tipo, responsabilidade e etapas do processo"
    >
      ...
      {visiblePhases.length > 0 && (
        <ol className="process-phase-track" aria-label="Etapas do fluxo">
          {visiblePhases.map((item, index) => {
            const completed =
              closed || (visiblePhaseIndex >= 0 && index < visiblePhaseIndex);
            const current = !closed && index === visiblePhaseIndex;
            return (
              <li
                key={`${item.phaseId}-${item.position}`}
                className={`process-phase-step ${completed ? "is-completed" : ""} ${current ? "is-current" : ""}`}
                aria-current={current ? "step" : undefined}
              >
                <span className="process-phase-marker" aria-hidden="true">
                  {completed ? <Check size={13} /> : index + 1}
                </span>
                <span className="process-phase-name" title={item.name}>
                  {item.name}
                </span>
                {index < visiblePhases.length - 1 && (
                  <span
                    className="process-phase-connector"
                    aria-hidden="true"
                  />
                )}
              </li>
      ...
      {visiblePhases.length === 0 && protocol.flowModeSnapshot === "SUGGESTED" && (
        <p className="process-flow-note">Sugerido não aplicado</p>
      )}
    </section>
  );
}

function ProcessMetaItem({
  icon,
  label,
  value,
}: {
  ...
  return (
    <div className="process-detail-meta-item" title={`${label}: ${value}`}>
      <span aria-hidden="true" className="process-detail-meta-icon">
        {icon}
      </span>
      ...
      {entries.map((entry) => (
        ...
        >
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              <Name db={db} userId={entry.actorUserId} />
            </p>
          </div>
...
function ProcessSummary({
  ...
  const elapsedDays = latestMovement
    ? Math.max(
        0,
        Math.floor(
          (Date.now() - new Date(latestMovement.createdAt).getTime()) /
            86_400_000,
        ),
      )
    : undefined;
  ...
  const routingOrigin = db.units.find(
    (item) =>
      item.id === (latestMovement?.fromUnitId ?? latestMovement?.actorUnitId),
  )?.name;
  return (
    <section className="panel mt-3 overflow-hidden">
      <SummarySection title="Informações do processo">
        ...
        <SummaryField
          label="Aberto por"
          value={<Name db={db} userId={protocol.createdById} />}
        />
        <SummaryField
          label="Data de abertura"
          value={dateTime(protocol.createdAt)}
        />
        {protocol.amountCents !== undefined && (
          <SummaryField label="Valor" value={money(protocol.amountCents)} />
        )}
        {protocol.contractNumber && (
          <SummaryField
            label="Número de contrato"
            value={protocol.contractNumber}
          />
        )}
        {protocol.biddingNumber && (
          <SummaryField
            label="Número de licitação"
            value={protocol.biddingNumber}
          />
        )}
        {protocol.legalProcessNumber && (
          <SummaryField
            label="Número de processo jurídico"
            value={protocol.legalProcessNumber}
          />
        )}
        {protocol.referenceNumber && (
          <SummaryField label="Número" value={protocol.referenceNumber} />
        )}
        <SummaryField
      ...
      <SummarySection title="Situação atual" divided>
        ...
        <SummaryField
          ...
          value={dateTime(latestMovement?.createdAt)}
        />
        <SummaryField
          ...
          value={
            elapsedDays === undefined
              ? undefined
              : `${elapsedDays} dia${elapsedDays === 1 ? "" : "s"}`
          }
        />
        ...
        <SummaryField
          ...
          value={
            latestMovement?.message ||
            (latestMovement ? eventLabel[latestMovement.kind] : undefined)
          }
        />
      </SummarySection>
    </section>
...
function EditProtocolDialog({
  ...
  const [dueAt, setDueAt] = useState(
    protocol.dueAt ? protocol.dueAt.slice(0, 16) : "",
  );
  const update = useMutation({
    mutationFn: () =>
      api.updateProtocol(ctx, protocol.id, protocol.version, {
        subject,
        description,
...
function TimelineDate({ value }: { value: string }) {
  ...
  return (
    <time
      dateTime={value}
      className="hidden w-16 shrink-0 text-right text-[11px] leading-4 text-slate-500 dark:text-slate-400 sm:block"
    >
...
function mapFlowPhasesToEvents(protocol: Protocol, events: ProtocolEvent[]) {
  ...
  orderedEvents.forEach((event) => {
    const explicitIndex = event.phaseId
      ? phases.findIndex((phase) => phase.phaseId === event.phaseId)
      : -1;
    if (explicitIndex >= 0) {
      phaseIndex = explicitIndex;
      result.set(event.id, phases[phaseIndex]);
      return;
    }
    if (
      event.kind !== "ABERTURA" &&
      event.kind !== "FASE_AVANCADA" &&
      event.kind !== "FASE_DEVOLVIDA"
    )
      return;

    if (event.kind === "ABERTURA") phaseIndex = 0;
    else if (event.kind === "FASE_AVANCADA")
...
function mapChecklistEvents(
  events: ProtocolEvent[],
  flowPhaseByEventId: Map<string, FlowPhaseSnapshot>,
) {
  ...
  orderedEvents.forEach((event) => {
    const eventPhaseId =
      event.phaseId ?? flowPhaseByEventId.get(event.id)?.phaseId;
    if (!eventPhaseId) {
      if (event.checklist?.length) result.add(event.id);
      return;
    }
...
function TimelineRow({
  ...
  }, [isLatest, latestEventId]);
  const latestFlowPhase = isLatest
    ? protocol.flowSnapshot?.phases.find(
        (phase) => phase.phaseId === protocol.currentPhaseId,
      )
    : undefined;
  const effectiveFlowPhase = flowPhase ?? latestFlowPhase;
  const manualPhase =
    !effectiveFlowPhase && e.phaseId
      ? db.phases.find((phase) => phase.id === e.phaseId)
      : undefined;
  const latestManualPhase =
    !effectiveFlowPhase && isLatest && protocol.currentPhaseId
      ? db.phases.find((phase) => phase.id === protocol.currentPhaseId)
      : undefined;
  const phaseName =
  ...
  const statusKey =
    ...
    (e.kind === "ABERTURA"
      ...
      : e.kind === "TRAMITACAO" ||
          e.kind === "REABERTURA" ||
          e.kind === "FASE_AVANCADA" ||
          e.kind === "FASE_DEVOLVIDA"
        ? "EM_ANDAMENTO"
        : e.kind === "CONCLUSAO"
          ? "CONCLUIDO"
          : e.kind === "ARQUIVAMENTO"
            ...
            : e.kind === "RECEBIMENTO"
              ...
              : "REGISTRO");
  const status =
    effectiveFlowPhase?.situationType?.name ??
    (statusKey in statusLabel
      ? statusLabel[statusKey as ProtocolStatus]
      : statusKey === "CIENCIA"
        ? "Ciência registrada"
        : eventLabel[e.kind]);
  const phaseColor =
    effectiveFlowPhase?.situationType?.color ??
    manualPhase?.color ??
    latestManualPhase?.color;
  const statusTone: Record<string, { badge: string; dot: string }> = {
    ...
    ARQUIVADO: {
      ...
      dot: "bg-violet-500 ring-violet-100 dark:ring-violet-950",
    },
    CIENCIA: {
  ...
  const badgeClass = phaseColor
    ? "border font-semibold"
    : statusTone[statusKey].badge;
  const dotClass = phaseColor ? "" : statusTone[statusKey].dot;
  const sourceUserId = e.fromUserId ?? e.actorUserId;
  const destinationUnit = e.toUnitId
    ? db.units.find((unit) => unit.id === e.toUnitId)
    : undefined;
  const destinationUserId =
    isLatest && e.assignmentId === protocol.currentAssignmentId
      ? protocol.currentAssigneeId ?? e.toUserId
      : e.toUserId;
  ...
  const canEditChecklist =
    ...
    isLatest &&
    acknowledged &&
    e.assignmentId === protocol.currentAssignmentId;
  return (
    <article className="relative flex gap-3 pb-2 last:pb-0 sm:gap-4">
      <TimelineDate value={e.createdAt} />
      <div
        data-timeline-dot
        className={`relative z-10 mt-3.5 hidden h-3 w-3 shrink-0 rounded-full ${phaseColor ? "" : "ring-4"} sm:block ${dotClass}`}
        style={
          phaseColor
            ? {
                backgroundColor: phaseColor,
                boxShadow: `0 0 0 4px ${phaseColor}22`,
              }
            : undefined
        }
      />
      {!isLast && (
        <span
          data-timeline-line
          className="absolute left-[5.375rem] top-7 hidden h-[calc(100%-0.5rem)] w-px bg-slate-200 dark:bg-slate-700 sm:block"
          aria-hidden="true"
        />
      )}
      <section className="min-w-0 flex-1 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-slate-700 dark:bg-slate-900">
        <div className="flex min-h-10 w-full items-center gap-2 px-3 py-1.5">
          <button
            ...
            aria-label={`${collapsed ? "Expandir" : "Recolher"} conteúdo de ${heading}`}
          ...
          >
            <span className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              <span
                className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${badgeClass}`}
                style={
                  phaseColor
                    ? {
          ...
          {!readOnly &&
            ...
            protocol.currentAssigneeId && (
              <span
                role="group"
                aria-label="Ciência e responsável"
                className="timeline-inline-actions"
              >
                <button
                  type="button"
                  className={`timeline-icon-action ${acknowledged ? "timeline-icon-action--done" : canAcknowledge ? "timeline-icon-action--pulse" : ""}`}
                  aria-label={
                    acknowledged
                      ? "Ciência registrada"
                      : "Dar ciência da tramitação"
                  }
                  disabled={acknowledged || !canAcknowledge}
                  title={
                    acknowledged
                      ? `Visualizado em ${dateTime(assignment.receivedAt)}`
                      : canAcknowledge
                        ? "Confirmar visualização e registrar ciência"
                        : "Aguardando ciência do destinatário"
                  }
                ...
                <button
                  type="button"
                  className="timeline-icon-action"
                  aria-label="Escolher outro responsável"
                  disabled={!canAssign}
                  title={
                    canAssign
                      ? "Designar outro responsável desta unidade"
                      : "Somente administradores da unidade podem alterar o responsável"
                  }
                  onClick={onAssign}
                >
          ...
          <button
            type="button"
            className="grid size-8 shrink-0 place-items-center rounded-md text-slate-500 hover:bg-muted dark:text-slate-400"
            aria-label={`Alternar conteúdo de ${heading}`}
            onClick={() => setCollapsed((value) => !value)}
          >
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
              <div className="flex min-w-0 flex-wrap items-center gap-3 text-sm">
                <span className="inline-flex min-w-0 items-center gap-2 font-medium text-slate-700 dark:text-slate-200">
                  <span className="grid size-7 shrink-0 place-items-center rounded-full bg-public-50 text-public-700 dark:bg-slate-800 dark:text-public-300">
                    <UserRound size={15} />
                  </span>
                  <span className="truncate">
                    <Name db={db} userId={sourceUserId} />
                  </span>
                </span>
                {canShowRoute && destinationUnit && (
                  <>
                    <ArrowRight className="shrink-0 text-slate-400" size={16} />
                    <span className="inline-flex min-w-0 items-center gap-2 font-medium text-slate-700 dark:text-slate-200">
                      <span
                        aria-label="Unidade de destino"
                        className="grid size-7 shrink-0 place-items-center rounded-full bg-emerald-50 text-emerald-700 dark:bg-slate-800 dark:text-emerald-300"
                      >
                        <Building2 size={15} />
                      </span>
                      <span className="truncate">{destinationUnit.name}</span>
                    </span>
                  </>
                )}
                {canShowRoute && destinationUserId && (
                  <>
                    <ArrowRight className="shrink-0 text-slate-400" size={16} />
                    <span className="inline-flex min-w-0 items-center gap-2 font-medium text-slate-700 dark:text-slate-200">
                      <span
                        aria-label="Responsável pela movimentação"
                        className="grid size-7 shrink-0 place-items-center rounded-full bg-emerald-50 text-emerald-700 dark:bg-slate-800 dark:text-emerald-300"
                      >
                        <UserRound size={15} />
                      </span>
                      <span className="truncate">
                        <Name db={db} userId={destinationUserId} />
                      </span>
                    </span>
                  </>
                )}
            ...
            <div className="border-t border-slate-100 px-3 py-2 dark:border-slate-800">
              ...
              <p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-600 dark:text-slate-300">
                {e.message ||
                  (e.kind === "ABERTURA"
                    ? `Processo cadastrado por ${db.users.find((user) => user.id === e.actorUserId)?.name ?? "usuário responsável"}.`
            ...
            <footer className="timeline-card-footer">
              {!readOnly && (
                <div className="timeline-card-actions">
                  ...
                  {allowFiles && isLatest && isActive(protocol) && (
                    <button
                      type="button"
                      className="timeline-action-button"
                      onClick={onDossier}
              ...
              <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
                <Clock3 size={13} />
                Registro em {dateTime(e.createdAt)}
              </span>
            </footer>
          </div>
        )}
...
function MoveDialog({
  ...
  const flowPhases =
    protocol.flowSnapshot?.phases
      .slice()
      .sort((left, right) => left.position - right.position) ?? [];
  const currentConfiguredPhase = flowPhases.find(
    (phase) => phase.phaseId === protocol.currentPhaseId,
  );
  const configuredPhase = reopen
    ? undefined
    : currentConfiguredPhase
      ? (flowPhases.find(
          (phase) => phase.position > currentConfiguredPhase.position,
        ) ?? currentConfiguredPhase)
      : flowPhases[0];
  const freeFlow = !configuredPhase;
  const suggestedFlow =
    protocol.flowModeSnapshot === "SUGGESTED" && Boolean(configuredPhase);
  const advancesConfiguredFlow = Boolean(
    currentConfiguredPhase &&
    configuredPhase &&
    configuredPhase.phaseId !== currentConfiguredPhase.phaseId,
  );
  const fixedDestination = configuredPhase?.destinationUnitId;
  const [phaseId, setPhaseId] = useState(configuredPhase?.phaseId ?? "");
  const [unitId, setUnitId] = useState(
    fixedDestination ?? protocol.currentUnitId,
  );
  ...
  const currentQuestions = currentConfiguredPhase?.checklistQuestions?.length
    ? currentConfiguredPhase.checklistQuestions
    : (currentConfiguredPhase?.checklistItems.map((text, order) => ({
        id: `legacy-${order}`,
        text,
        order: order + 1,
        required: true,
  ...
  const hasAttachmentRequirement = Boolean(
    currentConfiguredPhase &&
    (currentConfiguredPhase.requiresAttachment ||
      currentConfiguredPhase.requiredAttachmentTypes.length ||
      currentQuestions.some((question) => question.requiresAttachment)),
  );
  const requiredTypes = currentConfiguredPhase?.requiredAttachmentTypes ?? [];
  ...
  const requiredTypeNames = requiredTypes.map(
    (type) => attachmentTypeNames[type] ?? type,
  );
  const acceptedTypes = requiredTypes.length
    ? requiredTypes.join(",")
    : "application/pdf,image/png,image/jpeg,text/plain";
  const mutation = useMutation({
    mutationFn: async (
      productivity: { activity?: string; result?: string } = {},
    ) => {
      if (reopen)
        return api.reopen(ctx, protocol.id, protocol.version, {
          unitId,
      ...
      return api.forward(ctx, protocol.id, expectedVersion, {
        unitId,
        assigneeId: assigneeId || undefined,
        phaseId,
        message,
  ...
  const users = db.users
    .filter((user) => user.active && canReceiveWorkInUnit(db, user.id, unitId))
    .sort((left, right) => left.name.localeCompare(right.name, "pt-BR"));
  useEffect(() => {
    if (assigneeId && !canReceiveWorkInUnit(db, assigneeId, unitId))
      setAssigneeId("");
  }, [assigneeId, db, unitId]);
  const activePhases = db.phases.filter(
    (phase) => phase.active || phase.id === phaseId,
  );
  const phaseOptions = configuredPhase
    ? [{ id: configuredPhase.phaseId, name: configuredPhase.name }]
    : activePhases.map((phase) => ({ id: phase.id, name: phase.name }));
  const submitForward = (
    productivity: { activity?: string; result?: string } = {},
  ) => mutation.mutate(productivity);
  const selectFiles = (selected: File[]) => {
    ...
    if (
      ...
      selected.some((file) => !requiredTypes.includes(file.type))
    ) {
      setFileError(
        new Error(
          `Selecione somente os tipos exigidos: ${requiredTypeNames.join(", ")}.`,
        ),
      );
      return;
    }
    ...
    <>
      <Dialog title={title} onClose={onClose}>
        ...
        >
          {!reopen && (
            <section className="rounded-lg border p-4">
              <div className="mb-3 flex items-center gap-2">
                <h3 className="label !mb-0">Fase *</h3>
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${suggestedFlow ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200" : freeFlow ? "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200" : "bg-sky-100 text-sky-800 dark:bg-sky-950 dark:text-sky-200"}`}
                >
                  {suggestedFlow
                    ? "Fluxo sugerido"
                    : freeFlow
          ...
          <section className="rounded-lg border p-4">
            <h3 className="label mb-3">Destino</h3>
            <Field label="Unidade organizacional de destino *">
              <Select
                aria-label="Unidade organizacional de destino *"
                className="field"
                value={unitId}
                disabled={Boolean(fixedDestination)}
                onChange={(event) => {
            ...
            {fixedDestination && (
              <p className="mt-2 text-xs text-muted-foreground">
                Unidade definida pela próxima etapa do fluxo.
              </p>
            )}
          </section>
          <section className="rounded-lg border p-4">
            <h3 className="label mb-3">Prazo e destinatário</h3>
            <div className="grid gap-4 sm:grid-cols-2">
          ...
          <section className="rounded-lg border p-4">
            <h3 className="label mb-3">Texto da tramitação</h3>
            <Field label={reopen ? "Motivo da reabertura *" : "Descrição *"}>
              <textarea
                className="field min-h-28"
                maxLength={4000}
                value={message}
          ...
          {!reopen && hasAttachmentRequirement && (
            <fieldset className="rounded-lg border border-dashed border-primary/35 bg-primary/5 p-4">
              <legend className="px-1 text-sm font-semibold">
                Anexo obrigatório da fase atual
              </legend>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-medium">
                    Inclua o arquivo antes de tramitar para a próxima fase.
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {requiredTypeNames.length
                      ? `Tipos exigidos: ${requiredTypeNames.join(", ")}.`
                      : "PDF, PNG, JPEG ou TXT."}{" "}
                    Até 5 arquivos de 5 MB cada.
                  </p>
                </div>
                <label className="btn-secondary shrink-0 cursor-pointer">
                  ...
                  <input
                    ...
                    aria-label="Selecionar anexos obrigatórios"
              ...
              {files.length > 0 && (
                <div className="mt-3 divide-y rounded-lg border bg-card">
                  {files.map((file, index) => (
                    ...
                    >
                      <FileText className="shrink-0 text-primary" size={17} />
                      <span className="min-w-0 flex-1">
                        <strong className="block truncate text-sm">
                          {file.name}
                        </strong>
                      ...
                      <button
                        type="button"
                        className="btn-secondary !p-2 text-destructive"
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              className="btn-primary"
              disabled={mutation.isPending || (!reopen && !phaseId)}
            >
              {mutation.isPending
                ? "Aguarde…"
                : reopen
      ...
      {activityOpen && (
        <Dialog
          title="Registrar Atividade e Resultado"
        ...
        >
          ...
          >
            ...
            <Field label="Atividade principal desenvolvida *">
              <textarea
            ...
            <Field label="Resultado concreto / produto entregue *">
              <textarea
                className="field min-h-24"
                maxLength={4000}
                placeholder="Descreva o resultado entregue..."
                value={result}
            ...
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <button
                className="btn-primary"
                disabled={
                  mutation.isPending || !activity.trim() || !result.trim()
                }
              >
                Salvar e continuar
              </button>
...
function CompleteDialog({
  ...
  const [message, setMessage] = useState("");
  const mutation = useMutation({
    mutationFn: () => api.complete(ctx, protocol.id, protocol.version, message),
    onSuccess: onSaved,
  });
  return (
  ...
  return (
    <Dialog title={attachment.filename} onClose={onClose} wide>
      {error ? (
        ...
        <>
          {!url && text === undefined && <Loading />}
          {kind === "text" && text !== undefined && (
            <pre className="max-h-[65vh] overflow-auto whitespace-pre-wrap rounded-md bg-slate-950 p-4 font-mono text-sm text-slate-100">
              {text}
            </pre>
          )}
          {kind === "image" && url && (
...
function Attachments({
  ...
  const add = useMutation({
    mutationFn: (files: File[]) =>
      api.addAttachments(ctx, protocol.id, protocol.version, files),
    onSuccess: onChanged,
    onError: setError,
  });
    ...
    <>
      <section className="mt-4">
              ...
              <>
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    if (files.length > 5)
                      setError(
                        new Error("O limite é de 5 arquivos por operação."),
                      );
                    else if (files.length) add.mutate(files);
                    event.currentTarget.value = "";
                  }}
                />
                <button
                ...
                >
                  <Paperclip size={16} />
                  {add.isPending ? "Anexando…" : "Anexar arquivos"}
                </button>
              </>
            )}
        </div>
        ...
        {attachments.length ? (
          ...
          <Empty
            title="Sem anexos"
            detail="Adicione até cinco arquivos PDF, PNG, JPEG ou TXT de até 5 MB cada."
          />
        )}
      </section>
      {preview && (
...
function AssignDialog({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.assign(ctx, protocol.id, protocol.version, assigneeId),
    onSuccess: onSaved,
  });
  const users = db.users.filter(
    (user) =>
      user.active && canReceiveWorkInUnit(db, user.id, protocol.currentUnitId),
  );
  return (
